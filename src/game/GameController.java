...
public class GameController {
    ...
    public interface GameEventListener {
        ...
        int onCharacterSelection(int slotNumber);
    }
    ...
    /**
     * Handles end-of-battle victory: awards gold, increments wave.
     */
    ...
    public void saveGame() {
        ...
        saveManager.saveGame(state.getCurrentWave(), state.getGold(),
                state.getPlayers(), state.getInventory());
    }
    ...
    private void createParty() {
        ...
        while (state.getPlayers().size() < 2) {
            int classChoice = listener.onCharacterSelection(state.getPlayers().size() + 1);
            if (classChoice < 0) {
                return;
            }
